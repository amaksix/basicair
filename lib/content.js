...
function loadIndustryMarkets(locale) {
  const marketsDir = path.join(CONTENT_DIR, locale, "industry-markets");
  ...
  return fs
    ...
    .map((file) => {
      const slug = file.replace(/\.json$/, "");
      const data = JSON.parse(fs.readFileSync(path.join(marketsDir, file), "utf8").replace(/^\uFEFF/, ""));
      return { ...data, slug: data.slug || slug };
    })
    .sort((a, b) => (a.order || 0) - (b.order || 0));
}
...
function loadArticles(locale) {
  const articlesDir = path.join(CONTENT_DIR, locale, "articles");
  ...
  return fs
    ...
    .map((file) => {
      const slug = file.replace(/\.json$/, "");
      const data = JSON.parse(fs.readFileSync(path.join(articlesDir, file), "utf8").replace(/^\uFEFF/, ""));
      return { ...data, slug: data.slug || slug };
    })
    .sort((a, b) => {
